...
/*
  ...
  - anyMatch : 최소한 한 개의 요소가 매개 값으로 주어진 조건을 만조하는지 조사
...
//		.filter(g -> g.getGender().equals("남자"))
//	    .anyMatch(a -> a.getMath() + a.getEnglish() / 2 >= 70);
		
		.anyMatch(s -> s.getGender().equals("남자") && s.getMath() + s.getEnglish() / 2 >= 70);
		System.out.println(result); // Match 안에 전부 써도 됨.
	}
