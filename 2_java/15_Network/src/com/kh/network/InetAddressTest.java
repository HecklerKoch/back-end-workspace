...
/*
  ...
  - 클라이언트가 요청(Request)하는 내용을
    서버에서 처리하고 응답(Response)을 클라이언트류 보내
    
 IP 주소
 - 네트워크 상에서 컴퓨터를 식별하는 번호
 - XXX.XXX.XXX.XXX와 같은 형식으로 표현(xxx는 부호 없는 0 - 244 사이ㅣ의 정수
 - DNS (Domain Name System)를 통해 해당 도메인의 IP 주소를 알아낼수 있다.
 
 포트(Port)
 - IP 주소는 네트워크 상에서 컴퓨터를 찾을 때만 사용하고
   컴퓨터 내에서 실행하는 프로그램을 선택하기 위해 포트(Port) 사용
 - 클라이언트는 서버 연결 요청시 IP 주소와 포트 번호를 알고 있어야 한다.
 - 모드는 0-65535까지의 범위를 갖는다.
...
public class InetAddressTest {
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println("Localhostdml IP 주소 : " + ip.getHostAddress());
